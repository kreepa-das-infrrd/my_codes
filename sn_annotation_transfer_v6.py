"""
added the empty page empty in page index if page is blank
...
def generate_customer_field_data(key, data):
    ...
    return dict(correctedWordInfoList=corrected_word_info, fieldType='Single Value',
                confidence=confidence, correctedValue=value,
                correctedStartIndex=startIndex, correctedEndIndex=endIndex, pageNumber=page_no,
                fieldName=key.upper(), vendorType=vendor_type
                )
...
def create_annotation_data(annotations, annotation_values, key_prefix=''):
    ...
    for key, fields_data in annotation_values.items():
        ...
        if key == 'make_year_model' and fields_data.get('value'):
            ...
            create_annotation_data(annotations, fields_data['value'], key)
            ...
            annotations.append(generate_customer_field_data(new_key, fields_data))
...
def get_all_annotation_document():
    ...
    if os.path.exists("SN_data.json"):
        with open("SN_data.json", 'r') as fp:
    ...
    if not documents:
        ...
        all_scan_ids = ['15a99f39-1457-11eb-844d-a3c6e2460d86']
...
def generate_titan_annotation_data_for_document(document_json):
    ...
    for values in annotation_values:
        ...
        create_annotation_data(annotations, values)
